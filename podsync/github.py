...
logger.info(f"Using GitHub Token: ***{os.environ['GITHUB_TOKEN'][-5:]}")

class Github:
    def __init__(self, repo: str = os.getenv("REPOSITORY", "")) -> None:
        self.repo = repo
        assert self.repo, "Repo is not set"
        logger.info(f"Using repo: {self.repo}")
        self.releases = {}
    ...
    def upload_release(self, path: str | Path, release_name: str, *, clean=False):
        ...
        command = f"gh release upload --clobber -R '{self.repo}' '{release_name}' -- '{path.as_posix()}'"
        subprocess.run(command, shell=True, check=False)  # noqa: S602
        if clean:
