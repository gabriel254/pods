...
async def main():
    ...
    for entry in remote["entries"][:5][::-1]:  # 5 videos from oldest to latest
        vid = Path(entry["link"]).stem # type: ignore
        ...
        # get cover url
        if re.search(r'img src="(.*)"', entry["summary"]):  # type: ignore # noqa: SIM108
        ...
        if not conf.get("skip_audio", False):
            ...
            bilibili.update_pod_rss("audio", audio_items, feed=remote["feed"]) # type: ignore
        ...
        if not conf.get("skip_video", False):
            ...
            bilibili.update_pod_rss("video", video_items, feed=remote["feed"]) # type: ignore
        # Cleanup
