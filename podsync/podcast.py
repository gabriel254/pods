...
def generate_pod_header(feed_info: dict, config: dict, pod_type: str) -> dict:
    ...
    if "published" in feed_info:
        ...
        pub_date = now
    feed_url = f"https://github.com/{os.environ['REPOSITORY']}/releases/download/{pod_type}/{config['name']}.xml"
    return {
        "rss": {
            "@version": "2.0",
...
def generate_pod_item(
    ...
    if pod_type == "audio":
        enclosure = {
            "@url": f"https://github.com/{os.environ['REPOSITORY']}/releases/download/{release_name}/{filepath.name}",
